...
exports.handleGetTypeEffectiveness = handleGetTypeEffectiveness;
...
async function handleGetPokemon(args) {
    const { name } = args;
    if (!name) {
    ...
    if (data.moves.length > 0) {
        ...
        response += data.moves.slice(0, 8).map(move => `   • ${move.name.replace(/-/g, ' ')} (${move.type})`).join('\n');
    }
...
async function handleGetTypeEffectiveness(args) {
    ...
    let effectiveness = 1;
    defending_types.forEach((defenseType) => {
        effectiveness *= typeEffectiveness[attacking_type]?.[defenseType] ?? 1;
    });
    let description = '';
    if (effectiveness > 2)
        description = "It's extremely effective!";
