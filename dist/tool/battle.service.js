...
class BattleSimulator {
    ...
    performTurn(attacker, defender) {
        ...
        if (!move) {
            this.battleLog.push(`   ${attacker.name} has no usable moves!`);
            return;
        }
        ...
        if (move.power) {
            ...
            const isCritical = Math.random() < config_1.config.battle.critChance;
            const finalDamage = isCritical ? Math.floor(damage * config_1.config.battle.critMultiplier) : damage;
            const oldHp = defender.currentHp;
            defender.currentHp = Math.max(0, defender.currentHp - finalDamage);
            let logParts = [];
            if (isCritical)
                logParts.push("💥 A critical hit!");
            ...
            if (effectivenessText)
                logParts.push(effectivenessText);
    ...
    calculateDamage(attacker, defender, move) {
        ...
        let baseDamage = (((2 * config_1.config.battle.defaultLevel / 5 + 2) * move.power * (attackStat / defenseStat)) / 50) + 2;
        const isStab = attacker.types.includes(move.type);
        if (isStab)
            baseDamage *= config_1.config.battle.stabMultiplier;
        let effectiveness = 1;
        defender.types.forEach(defenseType => { effectiveness *= typeEffectiveness[move.type]?.[defenseType] ?? 1; });
        baseDamage *= effectiveness;
        const randomFactor = Math.random() * (config_1.config.battle.randomFactor.max - config_1.config.battle.randomFactor.min) + config_1.config.battle.randomFactor.min;
    ...
    calculateMoveScore(move, defender) {
        let effectiveness = 1;
        if (move.power) {
    ...
    run() {
        ...
        while (this.p1.currentHp > 0 && this.p2.currentHp > 0 && this.turn <= config_1.config.battle.maxTurns) {
            this.battleLog.push(`\n--- Turn ${this.turn} ---`);
            const hpBar = (p) => {
            ...
            if (first.currentHp > 0)
                this.performTurn(first, second);
            if (second.currentHp <= 0) {
                this.battleLog.push(`\n💀 ${second.name} has fainted!`);
                break;
            }
            if (second.currentHp > 0)
                this.performTurn(second, first);
            if (first.currentHp <= 0) {
            ...
            if (this.p1.currentHp > 0)
                this.processStatusEffects(this.p1);
            if (this.p2.currentHp > 0)
                this.processStatusEffects(this.p2);
            this.turn++;
        }
        if (this.p1.currentHp <= 0)
        ...
        if (this.turn > config_1.config.battle.maxTurns) {
    ...
    generatePreBattleSummary() {
        ...
        if (this.p1.stats.speed > this.p2.stats.speed)
            this.battleLog.push(`\n⚡ **Speed Advantage:** ${this.p1.name} is faster and will attack first!`);
        else if (this.p2.stats.speed > this.p2.stats.speed)
